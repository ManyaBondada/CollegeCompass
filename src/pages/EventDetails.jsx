...
const EventDetails = () => {
  ...
  useEffect(() => {
    ...
    if (currentEvents.includes(event.name)) {
      setIsSignedUp(true);  // Event is already signed up
    }
  }, [event.name]);
  ...
  const handleSignUp = () => {
    ...
    if (isSignedUp) {
      ...
      const updatedEvents = currentEvents.filter(item => item !== event.name);
      localStorage.setItem("events", JSON.stringify(updatedEvents));
    } else {
      // Add event to localStorage
      if (!currentEvents.includes(event.name)) {
        currentEvents.push(event.name);
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: "10px" }}>
        <Button
          ...
          style={{
            backgroundColor: isSignedUp ? "#C7A3FF" : "#EADAFF",
            color: "#000",
          }}
        >
